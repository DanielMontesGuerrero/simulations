...
SDL_Color get_color_for_ant(const Ant& ant) {
  switch (ant.type) {
    case 0:
      return Color::ANT_QUEEN;
    case 1:
      return Color::ANT_WORKER;
    case 2:
      return Color::ANT_REPRODUCTOR;
    case 3:
      return Color::ANT_SOLDIER;
    default:
      return Color::BLACK;
  }
}

void draw(SDL_Renderer* renderer, const Ant& ant) {
  auto color = get_color_for_ant(ant);
  SDL_SetRenderDrawColor(renderer, color.r, color.g, color.b, color.a);
  SDL_Rect aux = {
      .x = ant.x * Config::CELL_SIZE,
      .y = ant.y * Config::CELL_SIZE,
      .w = Config::CELL_SIZE,
      .h = Config::CELL_SIZE,
  };
...
void draw(SDL_Renderer* renderer, GameHandler* gamehandler) {
  if (gamehandler->is_executed_locally) {
    ...
    for (auto ant : gamehandler->langtonantGame.ants) {
      draw(renderer, ant);
    }
  } else {
    // not implemented
...
void draw(SDL_Renderer* renderer, SDL_Texture** status, SDL_Texture** inputtext,
  ...
  auto status_txt =
      ...
      "\tAnts: " + std::to_string(gamehandler->get_num_ants()) +
