...
GameHandler::GameHandler(int rows, int cols, bool is_executed_locally): is_executed_locally(is_executed_locally), client(Protocol::HOST, Protocol::PORT, Protocol::TYPE) {
	last_x = 0;
	last_x = 0;
	last_h = 0;
	last_w = 0;
...
void GameHandler::update(int x, int y, int w, int h){
	...
	if (is_executed_locally) {
		...
		auto buffer = client.send_message(Protocol::MESSAGE_EVENT, Protocol::EVENT_GET, vector<int>{y, y + h - 1, x, x + w - 1});
...
void GameHandler::update(){
	update(last_x, last_y, last_w, last_h);
}
...
void GameHandler::on_click(int i, int j){
	if (is_executed_locally){
		...
		client.send_message(Protocol::MESSAGE_EVENT, Protocol::EVENT_MOUSE_CLICK, vector<int>{i, j});
	}
}
...
void GameHandler::toggle_pause() {
	...
	client.send_message(Protocol::MESSAGE_EVENT, Protocol::EVENT_PAUSE, vector<char>{});
}

void GameHandler::increase_update_rate() {
	if (is_executed_locally) return;
		cerr << "Sending increase_update_rate request" << endl;
	client.send_message(Protocol::MESSAGE_EVENT, Protocol::EVENT_UPDATE_RATE_INCREASE, vector<char>{});
}

void GameHandler::decrease_update_rate() {
	if (is_executed_locally) return;
		cerr << "Sending decrease_update_rate request" << endl;
	client.send_message(Protocol::MESSAGE_EVENT, Protocol::EVENT_UPDATE_RATE_DECREASE, vector<char>{});
}
